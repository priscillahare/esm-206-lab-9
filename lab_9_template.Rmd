...
#looking at summary model seems incorrect. Bedrooms has neg coeffcient showing decrease in price with increase in bedrooms (doesnt seem right). Also price decreases when you switch from forclusre(reference) to regular (also seems wrong)

```

The next model: Exclude price per square foot since uses the response variable to calculate the predictor variable, and bedrooms/bathrooms (since these are largely explained by square footage...)
...
homes_lm3 <- lm(formula = Price ~ City + SqFt + Status, data = homes_sub)
...
full_aic <- AIC(homes_lm1) # it is only useful to report AIC when preseting or comparing multiple models
final_aic <- AIC(homes_lm3)

# Answer: which would you pick? 

# if you didnt consider anything else then you would choose the first model based off AIC alone but using judgement, the final model is better conceptually
...
predict_graph <- ggplot(predict_df, aes(x = SqFt, y = fit.fit)) + 
  ...
  geom_point(data = homes_sub, aes(x = SqFt, y = Price), alpha = 0.5) + #add original data set to predictions graph
  facet_wrap(~Status)
